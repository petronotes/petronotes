...
> К: я сам не понимаю...
> 
> И именно поэтому это опасное заблуждение - оно ведет к тому, что петрофизическую модель строят на неверных основаниях. На самом деле это не два способа делать одно и то же, а четыре разных уровня одной системы. Давайте разберем, как они соотносятся.
...
Петротип - принципиально не геологическая, а петрофизическая единица. Т.к. информативность методов ГИС не позволяет выделять литотипы, то для этой цели применяются петротипы – более крупные единицы классификации, включающие один или несколько литотипов.
...
Эта иерархия работает только если каждый термин используется строго на своем уровне. Как только литотипы начинают "выделять по ГИС", а фации - путать с литотипами, петрофизическая модель теряет геологическое основание и превращается в набор кривых. Поэтому в следующий раз, когда на совещании коллега скажет "петротип" - уточните: он имеет в виду петрофизический класс или условия осадконакопления? Ответ на этот вопрос покажет, одинаково ли вы понимаете модель.
