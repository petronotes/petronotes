В этих терминах всегда есть путаница. Но на самом деле это четыре разных уровня из одной системы. 

`10 минут чтения`

> Как-то у меня состоялся такой разговор на конференции с коллегой (К):
> 
...
Давайте разберем, как они соотносятся.

## Литотипы

(в англ.язычной литературе **lithofacies**)

это горные породы с набором установленных признаков, включающих гранулометрический состав, структуру, текстуру и фильтрационно-емкостные свойства.
...
В современной седиментологии идентификация литотипов основана на комплексе результатов исследования керна: гранулометрического, вещественного и минералогического состава горных пород, фильтрационно-емкостных свойств, а также фотографий и послойного описания керна.

![Пример выделения литотипов](./content/images/2026-08-02-Litho-petro-facies-sequence/1.png "Пример выделения литотипов по сопоставлению Кп и Кпр. Слева - викуловские отложения, справа - юрские отложения")

** Итог**, литотипы:

- описывают горную породу по керну 
...
(в англ.язычной литературе **petrotypes**)

принципиально не геологическая, а петрофизическая единица. Т.к. информативность методов ГИС не позволяет выделять литотипы, то для этой цели применяются петротипы – более крупные единицы классификации, включающие один или несколько литотипов.
...
Их подход основан на концепции глобальных гидравлических элементов (Global Hydraulic Elements, GHE), которая, в свою очередь, базируется на индикаторе зоны потока (Flow Zone Indicator, FZI). Если сказать проще, то FZI - это оценка качества пор для возможности течения флюида, рассчитанная по пористости и проницаемости.
...
![Выделение литотипов](./content/images/2026-08-02-Litho-petro-facies-sequence/2.png "Выделение литотипов по Кпр и Кп. Шифр точек Кво (пласт Ю1)")
...
![Выделение петротипов](./content/images/2026-08-02-Litho-petro-facies-sequence/3.png "Выделение петротипов по данным ГК и НК")
...
- используются для уточнения петрофизической модели - разные зависимости для разных петротипов

## Фации

(в англ.язычной литературе **facies association**)

это отложения или комплексы отложений, сформировавшихся в определенной физико-географической обстановке осадконакопления и характеризующихся специфическими литологическими, палеонтологическими и структурно-текстурными признаками.
...
![фации](./content/images/2026-08-02-Litho-petro-facies-sequence/4.png "Прибрежно-морские фации (R.W.Jones, 2021)")
![Примеры выделения](./content/images/2026-08-02-Litho-petro-facies-sequence/5.png " [Примеры выделения фаций на керне](https://scfh.ru/papers/sedimentologiya-klyuch-k-proshlomu-zemli/?ysclid=lw6bamazpj768669528)")
...
![Формы кривых](./content/images/2026-08-02-Litho-petro-facies-sequence/6.png "Формы кривых ПС/ГК и их седиментологическое содержание [C.G. St. C. Kendall 2003]")
...
- выделяются по комплексу: описание керна → кривые ГИС → распространение фаций между скважинами
- используются для уточнения межскважинной корреляции

## Секвенции
(в англ.язычной литературе **sequences**)
...
![секвенции](./content/images/2026-08-02-Litho-petro-facies-sequence/7.png "Системные тракты изменения уровня моря в секвенс-стратиграфии")
...
- используются для межскважинной корреляции

### Таким образом 

Все термины можно объединить в единую систему ранжируя их по масштабу выделения от меньшего к большему - литотипы (керн, микроуровень) → петротипы (петрофизические классы, видны по ГИС) → фации (палеообстановка, ГИС + керн) → секвенции (бассейновый уровень, сейсмика).

Эта иерархия работает только если каждый термин используется строго на своем уровне. Как только литотипы начинают "выделять по ГИС", петрофизическая модель теряет геологическое основание и превращается в набор эмпирических зависимостей. 

В следующий раз, когда на совещании коллега скажет "петротип" - уточните: он имеет в виду петрофизический класс или тип горной породы? Ответ на этот вопрос покажет, одинаково ли вы понимаете модель.
